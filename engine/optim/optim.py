...
class LESAM(Optimizer):
    """Loss-Equated SAM wrapper.

    LE-SAM uses a fixed first-order loss-space budget instead of SAM's fixed
    perturbation radius. Given gradients ``g``, the perturbation is scaled as
    ``sigma * g / ||g||^2`` so the linearized loss increase is approximately
    ``sigma``.
    """
